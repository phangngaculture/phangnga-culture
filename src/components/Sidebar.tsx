  ...
  return (
    <>
      ...
      >
        ...
        {onToggleDarkMode && (
          <div className="mx-3 my-1 px-3 py-2.5 rounded-xl bg-slate-800/80 border border-slate-700/60 flex items-center justify-between">
            <div className="flex items-center space-x-2.5 text-xs text-slate-200 font-medium">
              <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${darkMode ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-700 text-slate-300'}`}>
                {darkMode ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
              </div>
              <div>
                <div className="text-xs font-semibold leading-none">{darkMode ? 'โหมดมืด (Dark)' : 'โหมดสว่าง (Light)'}</div>
                <div className="text-[10px] text-slate-400 mt-0.5">ปรับธีมถนอมสายตา</div>
              </div>
            </div>
            <button
              type="button"
              onClick={onToggleDarkMode}
              className={`w-10 h-6 rounded-full p-0.5 transition-colors duration-200 ease-in-out cursor-pointer relative ${
                darkMode ? 'bg-amber-500 shadow-xs shadow-amber-500/30' : 'bg-slate-700'
              }`}
              title={darkMode ? 'ปิดโหมดมืด' : 'เปิดโหมดมืด'}
            >
              <div
        ...
        <div className="p-3 mx-3 my-2 rounded-2xl bg-slate-800/80 border border-slate-700/60">
          <div className="flex items-center space-x-2.5 mb-2.5">
            <div
              ...
              title="แตะเพื่อเปลี่ยนรูปโปรไฟล์ (กล้อง/AI Avatar)"
            >
              <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-orange-500 to-amber-500 text-white flex items-center justify-center font-bold text-xs shrink-0 overflow-hidden ring-2 ring-orange-400/40 group-hover:ring-orange-400 transition">
          ...
          {onOpenProfilePhoto && (
            <button
              ...
              className="w-full mb-2 flex items-center justify-center space-x-1.5 py-1.5 px-2 bg-orange-500/15 hover:bg-orange-500/25 text-orange-300 border border-orange-500/30 rounded-xl text-xs font-semibold transition cursor-pointer active:scale-95"
            >
